...
function statusClass(s: string) {
  return `badge badge-${s.toLowerCase().replace(/\s+/g, '-')}` as string;
}

function categoryClass(category: string) {
  return `badge badge-${category.toLowerCase().replace(/\s+/g, '-')}` as string;
}
...
function getThreatNameById(analysis: SecurityAnalysis, id: string) {
  return analysis.threats.find((t) => t.id === id)?.title ?? id;
}
...
function App() {
  ...
  const togglePriority = (p: string) => {
   ...
   /* Run real security analysis through the FastAPI backend */
  ...
  const handleExport = (format: string) => {
    ...
    if (format === 'json') {
      ...
      const rows = [['Threat', 'Category', 'Confidence', 'Affected Asset', 'Status']];
      analysis.threats.forEach((t) => {
        rows.push([
          ...
          t.affected_asset_ids
            .map((id) => getAssetNameById(analysis, id))
            .join('; '),
          t.status,
        ]);
      });
      ...
      analysis.threats.forEach((t, i) => {
        lines.push(`${i + 1}. ${t.title} [${t.category}]`);
          ...
          {activeTab === 'threats' && (
            ...
            >
              <div className="threat-card-header">
                <span className="threat-card-name">{threat.title}</span>
                <span className={categoryClass(threat.category)}>
                  {threat.category}
                </span>
              </div>
              ...
              <div className="threat-meta">
                <span>
                  <Server size={12} />
                  {threat.affected_asset_ids.length > 0
              ...
              <div className="threat-tags">
                <span className="threat-tag">{threat.status}</span>
                <span className="threat-tag">{threat.category}</span>
              </div>
          
              <div className="threat-evidence-section">
                <div className="threat-evidence-title">Description</div>
              ...
              <div className="threat-mitigation">
                ...
                This is an inferred potential threat. Validate the relevant
                security controls against the application's implementation.
              </div>
            </motion.div>
          ))}
            </motion.div>
